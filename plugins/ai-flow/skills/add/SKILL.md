...
- 目标目录有正在跑的 flow → 一般不再是拒绝的理由：本命令碰不到它的提示词，`state/` 也原样保留。CLI 会把这一点写在输出里。
- **唯一会拒的一种 `--force`**：项目侧 `config.json` 用自己的 `stages` 覆盖了插件的阶段表，而正在跑的 flow 停在一个插件阶段表里没有的 stage。重置会把阶段表换成插件那份，于是那个 stage 查不到——**后果不是报错而是静默失效**（引擎两条热路径都捕获异常并放行：新 session 不再注入 stage 提示词、PreToolUse 守卫全部 fail open、signal 不再被拦截，flow 永远推进不下去，只在 `flow.log` 留一行 ERROR）。CLI 会拒绝并给出三条出路，**原样转达**，不要替用户挑。
