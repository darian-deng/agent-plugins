...
| stage-3 | implement | script（无 gate，fail-closed） | 编排器串行派 fresh 子代理逐 ticket 实施：tdd→simplify→**三评审子代理并行**(双轴+correctness)→编排器裁/修→客观地板→**commit(subject 含 T号、一笔一票)**→qc marker(写在该票那条)→**真机票打 rm:pending**→勾[x]（实现在子代理、不涨主 session；commit 后置；frontier 分岔不问顺序、真机票不停） |
