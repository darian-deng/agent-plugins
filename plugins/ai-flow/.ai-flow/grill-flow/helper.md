...
- **「该不该问」和「怎么问」是两格，先走动作格**（2026-09 补）：`ask-before-asking.md` 只管怎么问；动作是加票 / 旁路修 / 改前置产物时，`mid-flight-ticket.md` / `side-fix.md` / `revision-protocol.md` 带「该不该问」的判据，而且常常判「不问」。实测一次：两格同时命中，主 session 只走了提问格，在正文里起「待拍板」块列三个选项后收尾——三步纪律全文就在注入的 stage-3 提示词里，漏的是岔路。所以 ① 触发锚从工具名改成回合形态（「这一回合结束后我是在等他回话吗」，正文摆选项请他选 = 调 `AskUserQuestion`），带豁免；② stage-3 岔路表那一格就地写「先走动作格」——必须写在注入页里，只写进 references 的话，走错格时根本读不到。
