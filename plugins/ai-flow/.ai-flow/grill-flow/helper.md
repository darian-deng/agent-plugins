...
- **「要不要问开发者」的完整判据住在 `references/ask-before-asking.md`，⛔ 别再挪回各 flow 自己的产物里**：它现在管两段——① 先查文档（查到就照做）；② **查完仍要问也要先派对抗审查**（自己形成带依据的推荐 → 派 fresh-context 子代理尽力推翻 → 收敛不了才 `AskUserQuestion`）。⚠️ **2026-09 改过一次语义**：原文写着「⛔ 不许它回「基本同意」」，那条指令实测是在命令子代理**必须找出不同意见** ⇒ 把它变成反驳生成器，造出来的反驳被当战果采纳，99 分的方案就这么被「优化」成 85 分。现在改成：**「攻不动」是合法结果**（要交代攻法与取证），**举证责任在挑战方、无取证不合并、平局保留原方案**，且**裁决方必须亲手复跑一条取证命令**。第二段是 2026-09 补的，**补的理由本身就是证据**：某次 flow 把这条规则写在自己的 `alignment.md` + `spec.md ## Decisions` + `tickets.md` 交接段**三处**且互相一致，新 session 仍连着违反两次——因为 stage-3 岔路表路由过去的是**这份定义文件**，而那三处产物都是按需读的，走岔路表到不了。⇒ **凡是「主 session 该怎么做」的纪律，落点是定义层的 references，不是 flow 实例的产物**；产物那侧只做落盘备份（交接段就地写全、不给指针）。
- **先认出「我在提问」，再让动作格判问不问**（2026-09 补）：实测一次，主 session 在正文里起「待拍板」块列三个选项后收尾，事后自述「没调 `AskUserQuestion`，连自己都没察觉在提问」，还以为在遵守用户全局规则里的起块格式。违规前它只读过 `reentry.md`，岔路表**哪一格都没走**；三步纪律全文就在注入的 stage-3 提示词里。所以 ① 识别线索写进注入页：岔路表那一格是「想停下来问开发者，**含正文摆选项后收尾**」；`ask-before-asking.md` 的触发锚是回合形态（「这一回合结束后我是在等他回话吗」），带豁免；② 动作是加票 / 旁路修 / 改前置产物时，那一格的判据**优先**：判不问就不问（安全红线除外），判 L1/L2 必须停就直接停、⛔ 不再走三步。⚠️ 曾写成「它判该不该问，本格只管怎么问」，对抗审查指出那会把 L2 必停送回三步，子代理一同意就继续推进，违反 `revision-protocol.md` 的「L1/L2 必须停」。
- **交接只许一个落点：`tickets.md` 的 `## 🔴 重入交接`**（2026-09 补）：实测 6 条 flow 里 3 条另建了 `handoff.md` / `HANDOFF.md`，其中 2 条的交接段是 0 段，入场表 ⑥⑦⑧ 整片缺失而没有任何东西报错；在第二份文件首行写「先读 tickets.md」也被实测无效。现由 `handoff.md` 禁令 + 收尾自检 + `gate-stage-3.cjs` 一条不占编号的断言（产物目录下不得有 `handoff*.md`，排在 ① 之前）三处兜住；`reentry.md` 开头要求入场先用 `read-section.cjs` 读完交接段，⛔ 不许用 grep 切票号代替。
- **异步派发的空转有两种，解法相反，别用错**（详见 `references/subagent-lifecycle.md` §一）：
  - **子代理往下派孙代理 → 🔴 一律禁止**：子代理没有「挂起」态、也不能被孙代理唤醒，派完只剩一轮轮 `Bash: true` 空烧着等，而每个空转回合都要为当时的全部上下文付一次全额。实测 157/311 次派发跑在后台 → 422 个纯空转回合，约占全流程 5%。⚠️ 这里曾写「一律 `run_in_background: false`」——那个参数在这一层**不生效**（实测显式传 false，工具结果仍是 async，它也不在 Agent 工具的 schema 里），要求它反而让这条规则被违反了 9 次。⇒ 该做的事子代理自己顺序做完，或上收给主 session。
...
- **stage 提示词有硬预算：渲染后 ≤ 10,000 字符**（宿主的内联注入上限，按字符不按字节；实测夹逼出来的，见 `src/lib/prompt-render.ts` 的 `INLINE_INJECTION_BUDGET`）。**超了不会静默丢正文**——引擎的兜底是：把**渲染后**的提示词落盘到 `{{flow_root}}/state/current-prompt.md`，并改注入一条「本 stage 的提示词是 N 字符、超过上限，**因此它没有随这次注入送到你手上**，现在立刻 Read 它」的指令（`injectableStagePrompt`，v0.63.0 起；副本带 `<!-- ai-flow: stage=<id> -->` 头，防读到上一个 stage 的旧件）。⇒ **超预算的代价是一次 Read 往返，不是规则静默消失。**⚠️ 仍然要守预算，理由是那次 Read 会被跳过或读一半；⛔ 但不要把它当成「加一个字就会有规则静默失效」——那是引擎兜底之前的行为，已经不成立。预算本身由 `tests/stage-prompt-budget.test.ts` 执行。
...
- **质量把控齐**：per-ticket Standards/Spec/correctness 三轴评审（Standards 轴兼管原 simplify 的四类机械修）+ 注释清理 + 客观地板（假绿检测/枚举负空间/回归）；stage-2 对抗性方案审查；收尾组装审（Standards / Spec 两轴按规模可跳过）+ 安全专项（强制、任何规模都跑；对抗立场 + 阻塞项独立复核）；集中沉淀。

- **规模自适应目前只有两处，判据都不是「diff 多大」**（本节成本均为**含子孙代理**的子树口径。⚠️ **这批数字全部采自旧形态**——三评审与四轴当时是派孙代理跑的、各占一份独立子上下文；改成同一个代理顺序走完之后，省掉的是空转回合，**不是 token**，而顺序走会让后一轴带着前一轴的输出 ⇒ 总账往哪边走要重新攒样本才知道。⛔ 别拿这些数字去论证新形态更省或更贵）：① `scripts/schedule.cjs` 选执行单位（票少时自动落回一票一树，实测 ≤9 票时车道模式根本算不赢）；② stage-4 组装审的 Standards / Spec 两轴按规模可跳过（安全专项永不跳）——**判据以 `references/assembly-review.md` 的「派几个先算一次」为准，本页不复述**（阈值与命令一改就会漂）。要点只有两条：判相交**用实际改动文件、不用票面 `Touches` 声明**；跳过只消掉「同文件不同区段」那一类交界面，**「跨票反向读取对」没有机器判法、必须转环节 C 人审**。
  ⚠️ **`/simplify` 整步已删除**（开发者 2026-09-03 当面拍板），那四类并入 Standards 轴、由它就地修，`qc-metrics` 的 `simplify=` 换成 `fixed=`。留档：它在最小的一张票（13 行 diff）上四个轴全部返回「无 finding」、花 0.577M；⛔ 这不是「可以恢复它」的伏笔。
  ⚠️ **减配仍然只能靠事后样本判，⛔ 不要按 diff 行数减配**——**票的大小事前预测不了**（`stage-2.md` 记着：票面声明的改动路径数与实际工作量秩相关 0.15、字数 0.12、六特征回归交叉验证 R² 为负）。所以每票留一行 `qc-metrics:`——由质量链代理回报、主 session 抄进 **tickets.md**（stage-3 记账第 3 项），攒够之后 `grep -rn qc-metrics docs/grill-flows/` 一次性回答「线划在哪」。⛔ **别写进 commit body**：stage-4 的 squash 会 `git reset` 摊平 + 全新 message 重提 + 删票分支，实测那之后 `git log --format=%b | grep -c qc-metrics` = 0。划错线的失败方式是静默的。
  ⚠️ **第二段有个规模无关的成本地板**：六对配对回归的截距 3.22M，最小那张票实测 3.71M（是它实施段 0.846M 的 **4.38 倍**）。所以小票的「装置/实干」比天然高，这不是 bug，是拆分 + 三评审 + 四轴的固定入场费（同上：按旧形态测的）。三个最小票平均 4.01×，三个最大票平均 0.95×，六对区间 **0.42×–4.38×**（比值 vs log10(实施成本) 的 r² = 0.72）。
...
| ID | 名称 | 完成方式 | 关键机制 |
|----|------|---------|---------|
| stage-1 | grill（需求对齐，domain-aware） | **gate** | grilling 一次一问 + wayfinder 迷雾子模式 + research/prototype detour + 待定点按**猜错的代价**排序（⛔ 不是按不确定度）、前置力气按信息类型分配（目标类死磕、输入类记成 `⚠假设` 撞上再问） + 结账前跑**冷读覆盖度审查**（fresh-context 子代理只读 `alignment.md`、禁读对话与需求原文，报回每一处「到这里必须猜」，落 `## 冷读缺口` 段且每条都要有下落）——自评覆盖度不可信（业界基准里最好的模型只问出 32% 的缺口），而开发者确认不了自己不知道的东西 |
...
├── alignment.md         # 需求/范围/决策/术语/暂缓/沉淀候选/功能覆盖缺口(替换迁移型)/冷读缺口(必跑,stage-1)
...
.ai-flow/grill-flow/state/   # 引擎维护：active.json / signal / mark-base / flow.log
...
- **宿主只需允许主 session 派子代理**（depth-1）：stage-3 的一票走两段（实施 / 质量链），加上主 session 另派的注释清理，**全部是 depth-1**。
  ⚠️ **这条依赖曾经强得多**：旧契约让质量链子代理自己往下派三评审、并让 `comment` 自己 fan-out，于是整套质量链吊在「宿主允不允许子代理再派子代理」上。现在不吊了——子代理一律不许往下派（那一层结构性异步且无开关，理由见 `subagent-lifecycle.md`），三评审改成质量链代理自己顺序审，注释清理上收主 session。**顺带删掉了两条旧边界**：「嵌套是否可用 + dispatch 里要带降级路径」（不再派嵌套）与「重复角色 spawn 会被分类器拦」（不再有同层三个近似角色）。剩下两条仍然成立：
  - **并发有上限**（默认 20 上下，可由 `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` 覆盖）。改成 depth-1 之后每票同时在跑的只有一个（实施 / 质量链 / 注释清理三者之一），并发压力比旧形态（每票 1 实施 + 3 评审 + `comment` 的 fan-out）低一个量级，`schedule.cjs` 算出的并发基本跑得满。
  - **子代理拿不到 `AskUserQuestion`**。契约里「人在环只在主 session 那侧」这条因此不只是分工，而是宿主约束——子代理遇到要拍板的事只能回报，不能自己问。
- **context 收尾阈值（`config.json` 的 `context` 段）**：`wrap_up_at_pct: 60`——**整个 context 段就这一个键**（旧的 `warn_at_pct` / `block_at_pct` 两级、以及在两级之间节流重复提醒的 `rewarn_delta_pct` 全已删；只提示不做事的那一级是噪音）。越过阈值后 `context_wrap_up`（`{ at_pct }`）会**锁进 flow 状态**，同时发生两件事：①注入一次「开始为 /clear 收尾」的简报——**只发这一次，此后永不重述**（`at_pct` 冻在首次撞线那个数；常驻提醒由下面那条拒写文案本身承担，它每次都会说清收尾已开始、哪些路径还能写）；②此后主 session 对**代码**的写入一律被拒，只有本 flow 自己的 `docs_paths`（`docs/grill-flows/`）仍可写——这是刻意留的，**拒写的目的是不让你在退化的 context 上继续产出，不是堵死安全退出**。所以每个 stage（包括 `write_scope: unrestricted` 的）都要配 `docs_paths`（grill-flow 5 个 stage 全配了）；⚠️ 没配的 stage 没有安全出口，引擎在那种 stage 上**一个写入都不拒**（拒了连交接块都写不下去），只剩简报，收尾全靠自觉。⚠️ **这道闸只锁主 session**：子代理（hook input 带 `agent_id`）既不参与测量、也不被它拦——上下文是独立的，而「把活交给 fresh context」恰恰是 context 退化时的处方。实测这条曾经缺失（测量侧修了、执行侧漏了）：主 session 在 61% 锁定后派子代理修 7 项，子代理在自己只有 75K（窗口的 7.5%）时被拒，一项做了一半、留下一个没人 import 的新文件。⚠️ 它是给模型的信号、不是安全边界——`Bash` 不在拦截清单里，`cat >` / `sed -i` / `git commit` 一直放行。
...
- **内置命令**：无。⛔ `/simplify` 曾用于 stage-3 per-ticket 机械型质量修，**2026-09-03 由开发者拍板整步删除**，那四类已并入 Standards 轴。三个评审轴都不依赖任何内置 slash 命令，任何仓库通用（见 `references/per-ticket-review.md`）。
