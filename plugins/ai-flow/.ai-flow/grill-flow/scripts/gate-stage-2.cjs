...
for (let k = 0; k < idxs.length; k++) {
  ...
  for (const bl of blockLines) {
    ...
    if (segs.length < 2) {
      err(self + ' 的 ' + acId + ' 没写成可跑的形状（整条里找不到 " => " 分段）: ' + bl.trim() + AC_SHAPES);
      process.exit(FAIL);
    }
    // 只断言**最后一段**：中间可以是命令、也可以是几段铺垫，那由写的人决定；
    // 但结尾必须落在「能拿去比对的东西」上，否则这条 AC 到质量链那儿依然无从跑起。
    const last = segs[segs.length - 1].trim();
    if (last !== 'rm' && !/^`[^`]+`$/.test(last)) {
      err(self + ' 的 ' + acId + ' 最后一段既不是反引号包起来的期望输出片段、也不是字面 `rm`: '
