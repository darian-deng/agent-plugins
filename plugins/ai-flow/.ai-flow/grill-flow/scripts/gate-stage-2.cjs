...
for (let k = 0; k < idxs.length; k++) {
  ...
  // ── AC 子项的**形状**断言 ──────────────────────────────────────────────────
  // 🔴 这道门查的是**形状，不是真伪**。`AC1: 冒烟 => `echo ok` => `ok`` 照样过：它只看
  //    「有没有写成一条能跑的命令 + 一条能比对的期望片段」，不会去跑那条命令、更不会去判
  //    这条 AC 验的是不是该验的东西。真伪由质量链逐条真的跑一遍来判，不归这道门。
  //
  // 那它凭什么存在：**时机**。今天 AC 是自由散文——`scripts/gate-stage-3.cjs:141` 明写
  // 「缩进的 AC 子项不参与判定」，也就是说票面上的 AC 从写下那一刻起，一路到 stage-3
  // 都没有任何东西看过它一眼。等 stage-3 才发现某条 AC 根本写不出可跑命令，票已经开出去、
  // worktree 已经建好、子代理已经在做了，改 AC 等于回炉重开；卡在 stage-2，票面还在手里，
  // 改一行字就完了。所以这道门买的不是「AC 是对的」，是「AC 还来得及改的时候就长成可跑的形状」。
  //
  // ⚠️ fail-closed（`process.exit(FAIL)`），与本文件其余断言同形态。
  // ⚠️ 没有任何 `AC<n>:` 子项的票**不报错**：「AC 是不是必需」是另一条规则，要加得单独
  //    论证。别在形状门里顺手把它一起管了——一道门同时管两件事，以后想松其中一件就只能
  //    整道关掉。
  // 行锚（`^\s*[-*]?\s*AC\d+:`）与上面 `Blocked by` 那条同理：无锚的 /AC\d+:/ 会吃到
  // 散文里提到的「AC1: 的含义是…」，把说明文字当成 AC 子项报错，而真正那条在它后面。
  // ⚠️ bullet 类要放宽到 `-` `*` `+` 与有序列表（`1.` / `1)`）：漏掉的那几种不会报错，会**静默
  // 跳过**——`if (!ma) continue;` 把它当成「不是 AC 子项」，于是把 AC 编成 `1. AC1: 这条先留空`
  // 的人拿到全绿，而这道门存在的全部理由就是拦住它。
  const AC_ITEM = /^\s*(?:[-*+]|\d+[.)])?\s*(AC\d+)\s*:\s*(\S[^\n]*)$/;
  const AC_SHAPES = '\n    两种合法形状，二选一：'
    + '\n      AC<n>: <一句话> => `<命令>` => `<期望输出的字面片段>`'
    + '\n      AC<n>: <一句话> => rm      ← 只有真机能验的（契约明禁子代理起 Electron / 跑长命令，'
    + '这类 AC 写不出可跑命令）'
    + '\n    ⚠️ 这道门只查形状、不查真伪：`AC1: 冒烟 => `echo ok` => `ok`` 同样能过。'
    + '它买的是「stage-2 还来得及改的时候，AC 就已经是可跑的形状」——'
    + '等 stage-3 发现 AC 不可验，票已经开出去了。真伪由质量链逐条跑。';
  for (const bl of blockLines) {
    const ma = AC_ITEM.exec(bl);
    if (!ma) continue;
    const acId = ma[1];
    const segs = ma[2].split(' => ');
    const last = segs[segs.length - 1].trim();
    // `=> rm` 只需两段（一句话 + rm）：那类 AC 本来就没有命令可写。
    if (last === 'rm') { if (segs.length >= 2) continue; }
    // ⛔ 其余一律要**至少三段**（一句话 => 命令 => 期望片段）。只要两段就放行的话，
    // `AC1: 描述 => `ok`` 能过 —— 而那正是「写了期望、忘了写命令」的形态，也正是这道门最该在
    // stage-2 拦下的那一种；只剩「一个 => 都没有」会被抓，等于形同虚设。
    // ⚠️ 文案要分两种，否则会指错原因：`AC1: 描述 => rm（需要真机）` 与 `AC1: rm` 走的也是
    // 这里（`rm` 后面多了字 / 压根没有 ` => `），而「忘了写命令」跟它们无关——旧版那句
    // 「既不是反引号包起来的期望输出片段、也不是字面 `rm`」对这两种才说得准。
    if (segs.length < 3) {
      const looksRm = /(^|[\s（(])rm([\s（(]|$)/.test(last);
      err(self + ' 的 ' + acId + (looksRm
        ? ' 的末段不是**字面** `rm`（`rm` 后面不能再带字，要解释写在前面那段）: '
        : ' 分段不足（`=> rm` 之外要 <一句话> => <命令> => <期望片段> 三段，只有两段多半是写了期望、忘了写命令）: ')
        + bl.trim() + AC_SHAPES);
      process.exit(FAIL);
    }
    // ⚠️ 判据是「最后一段里**含有**反引号片段」，不是「整段恰好是一个反引号片段」。后者会误拒
    // 一批合法写法，实测三种：`… => `1 passed`（本票新增）`、句末带句点、期望有两片
    // （`` `a` 和 `b` ``）—— 而报错文案还写着「不是反引号包起来的」，读的人看着自己明明写了
    // 反引号，只会以为门坏了。
    if (!/`[^`]+`/.test(last)) {
      err(self + ' 的 ' + acId + ' 最后一段里找不到反引号包起来的期望输出片段: '
        + JSON.stringify(last) + AC_SHAPES);
      process.exit(FAIL);
    }
  }
}
