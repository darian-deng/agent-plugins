...
**接缝声明**：三评审、`/simplify`、`comment` skill 全由**质量链代理**自己派/自己调，**主 session 不另派**；它只把判断型/决策型/安全型 findings 回报上来，由你裁。

⛔ **派发 prompt 里给子代理写死：它自己往下派孙代理（三评审、`comment` fan-out）必须同步（`run_in_background: false`）。** 子代理不能被唤醒，丢后台只剩一轮轮 `Bash: true` 空烧着等，每个空转回合都按当时的全部上下文全额计费——实测 **157/311 次派发跑在后台 → 422 个纯空转回合**，最多一张票烧掉 178 轮。**你自己派子代理反而可以丢后台**（主 session 会被唤醒，这是你有而它们没有的能力），那样你才腾得出手做下面那件事。
...
⛔ **`rm:pending` 必须排在 `qc:done` 之前**：重入相位表用 `qc:done` 当「记账已完成」的锚，如果 `rm:pending` 在它之后写，恰好在两者之间 /clear 就会被「有 `qc:done` 无 `[x]` → 补勾」这条路径**永久跳过**真机登记，而这个丢失是静默的（stage-4 只检查该段无 `rm:pending` 残留，空清单天然满足）。
