...
5. **切 tickets**（照 to-tickets）→ `tickets.md`：tracer-bullet 垂直切片，每片穿透各层、可独立验证/commit。**prefactor 前置**（要改处先重构才好改 → 排第一个 ticket）；wide-refactor 用 expand→分批迁移→contract。每条 **ticket 级** `- [ ] T<n> <标题>` + `delivers:` + `Blocked by:` + `Touches:`（ticket 内 acceptance criteria 用 `AC:` 前缀子项，不参与 frontier/门）。
   ...
   - **枚举型断言要写清穷尽方式**：「散落在其它四处」这类必须给出可复跑的判据（`grep -rn '<pattern>' <dir>` 及当时命中数）。只给一份手抄清单，实施方会照清单做完就停；⛔ 也不许写成「以该判据的当前全部命中为准」而不给数——那正是下面那道门要堵的占位写法。
   - ⛔ **批量型票的计数前置门（数不出来就不许切这张票）**：凡是 `delivers` 属于「把一批同类东西搬走 / 改掉 / 删掉」（迁移、批量重命名、消费方批改、清理一类违例）的票，`AC:` 里**必须**带一条可复跑的计数判据**及切票时真跑出来的命中数**（`grep -rn '<pattern>' <dir>` → N）。数不出来 → **退回把枚举做完**（走 `revision-protocol.md`），不许用「剩余零散」「同上」这类描述占位。
