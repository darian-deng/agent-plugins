...
🔴 **那一段顺带就是「对抗审查有效」的判据本身，不是安全专项碰巧这么跑**：它站得住，是因为它有一个**可数的枚举面**——枚举了什么面、枚举了多少个、逐条结论是什么，三问都答得出。答不出枚举面的对抗审查是意见交换，⛔ **不该有推翻权**（「几个代理都同意」不构成证据）。这条纪律的正式落点是 `references/ask-before-asking.md` 的「派对抗审查时的三条实测纪律」节，本节的安全专项是它点名的样板。⛔ 别把那节的细节复述到这里——理由同上面对安全红线五类的处理：复述必漂。
...
**每次清理跑完，在 review.md 记结果之前先跑这一条核实**（两次调用各跑一次）：

```sh
git status --porcelain | cut -c4- | grep -E '\.(test|spec)\.|(^|/)__tests__/|(^|/)tests?/' || echo 'OK: 测试文件零改动'
```

非空即违规：`git checkout -- <命中的那些测试文件>` 撤回（未追踪的新文件直接删掉），**重跑到打印 `OK`**，并在 review.md 写一句撤了什么。⛔ **pattern 逐字照抄，不许重打**（⛔ `cut -c4-` 也不许省：`--porcelain` 的 3 字符状态前缀会让 pattern 里那两个 `(^|/)` 锚全部失效，实测零命中）——它和 `quality-chain.md` 第 3 步那条完全相同，那边实测被重打缩水过两次。⚠️ 这道门不是凭空加的：`quality-chain.md` 第 3 步记着一次真实穿透，清理代理动的唯一文件就是一个 `__tests__/` 下的测试文件，而当时那条命令恰好被换掉了、门没响。它**不依赖任何基准 sha**，所以在 `git reset` 之后的本环节照样成立。

⛔ **别拿 `git diff ORIG_HEAD` 当基准去核「非注释行改动为 0」。** 本环节跑在 `git reset` 之后，本 flow 的**每一个新文件**都不在 index 里，`git diff ORIG_HEAD` 会把它们全部报成 `deleted file mode`（实测可复现），那条断言对每个新文件当场假阳性。（`git stash create` 也不行：实测它的快照**不含未追踪文件**。）⇒ **这里目前没有可用基准，「非注释行改动为 0」这条在本环节核不了**，只有上面那条测试文件 grep 是确定可用的。⛔ 不要为此发明一套快照机制——那需要设计，不在本环节范围里。
