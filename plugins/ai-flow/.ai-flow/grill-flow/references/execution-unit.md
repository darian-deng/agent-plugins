...
算成本时别漏了**实际峰值并发**：每票 = 1 实施 + 3 评审 + `comment` skill 自己的 fan-out。⚠️ **fan-out 的并发上限去 `comment` skill 里取当前值，别照抄任何文档里的数字**——它改过（曾是「每文件一个子代理」，现在是「一子代理认领一批」），复述过的数字都会漂。3 票同时走到注释清理那步，峰值可达数十个 agent。真正的上限往往是这个，不是票数。
