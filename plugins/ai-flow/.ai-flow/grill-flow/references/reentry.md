...
⛔ **不许用 `grep -n 'T<n>'` / `sed -n '<起>,<止>p'` 切票号代替它**——那些图案**全部落在票列表区**，
交接段整段被跳过**而退出码是 0**。实测一次：整轮只用切片访问 `tickets.md`（15 次，无一次触及
开头），第 ⑧ 格一眼没看，当轮就违反了它写着的那条纪律。
...
| 同上，**但票面没有 `cm:done`** | 有 | 已交付，**注释清理还没做** | **先派注释清理，再 close**：主 session 派一个 fresh-context 子代理调 `comment`（范围 = 本票那笔 commit 的改动），它改完由你 `git -C <WT> commit --amend` 折回本票那笔，然后在票面写 `cm:done`。⛔ **直接 close 就补不回来了**——ff 之后 `--amend` 不可用，那一票的注释清理只能另起一笔（破「一票一 commit」）或整个留到 stage-4。⚠️ **上一行的判据对这一格同样成立，所以先看 `cm:done` 再决定要不要 close**；而**上收之前收口的票面本来就没有这个字段**，那些票按上一行走（机器门在断言② 同趟检它，缺了只警告、不阻断，正是为了这批老票）|
| 同上，**但票面写着「裁定要修 / `qc-round-<n>` 待派 / 待修 findings」这类字样** | 有 | 质量链已交付、主 session 已裁完，**复派还没发出去** | 🔴 **不是去回合** —— 按票面那几条裁定**复派质量链（形态丙）**，prompt 里把「已裁定维持现状」的条目逐条列出并写明 ⛔ 不要重开。⚠️ **上一行的判据（未合回来 + 不含 `[partial]`）对这一格同样成立，所以先看票面再看 git**：2026-09-01 T24 与 T28 各踩一次，照上一行走会把一张**裁定未落地**的票 `--ff-only` 合进需求分支，而且不报任何错 |
| 任一格，且票面写着「交付时没有 sync」或 `worktree.cjs status` 报 `是否HEAD后继:NO` | 有 | 该树基线比需求分支旧 | **先 sync 再重跑地板**，⛔ 不许拿旧基线的绿当结论（地板会绿，但它验的不是将要合入的那个状态——静默失败形态）。⚠️ **责任方按触发方式分**：主动 sync（本格）由主 session **写进复派 prompt** 让质量链代理做；被动 sync（`close` 报错退回）由质量链代理按 `quality-chain.md` 的「归并前自适配」自己做。⛔ 别只依赖后者——它的触发时机是「close 失败之后」，覆盖不到本格 |
| 树干净、`ahead:1`，且 `worktree.cjs status` 的**静默 < 30 分钟**（⚠️ 本行与上面「已交付未回合」「待复派」两行**会同时匹配同一个输入**——本行只回答「它停了没」，那两行回答「接下来做什么」，⛔ 别拿本行当动作依据） | 有 | ⚠️ 可能仍有代理在飞，但**树干净这一格通常安全** | 判据：质量链的 `--amend` 在流程末尾，中途改文件会让树变脏 ⇒ **树干净 + 有本票 commit** 基本等于「它已经收尾」。⛔ 但仍要先核首行回报（`commit: <sha>` 且等于 `rev-parse HEAD`）再动手；核不到回报就按本文件「回报丢失」那条走 |
| 在 `wt/<flow_id>-T<n>` 上、未合回来 | 无（分支还在） | 已交付、工作目录被清掉 | 上面那步 `git worktree prune` 本身就会制造这个状态（有人手动删过目录）。`git worktree add <path> <branch>` 复用该分支恢复，再按「已交付未回合」那一行走。**别重派**——重派会撞「分支已存在」 |
| 在主分支上 | 还在（`close` 里 ff 成功但 remove 失败） | 已回合未拆 | `git worktree remove` 补拆，再按下一行继续 |
| 在主分支上、无 `qc:done` | 无 | 已回合未记账 | 补主循环第 6 步的记账，不重跑质量链（⚠️ 真机三态不在这里——它在第 5 步、close 之前就落了，`close` 拒缺标记的票） |
