...
5. **编排器裁 findings**：
   ...
   - **注释清理（每票 CR，commit 前必做）→ 编排器显式调用 `comment` skill**（`/ai-flow:comment`，范围=该票的**未提交已追踪改动 ∪ 该票新建的未追踪文件**——两半边的取法与理由见 skill，此处不复述）：它自带 grep 符号候选 + sonnet 语义判断（含文字类进程指代/冗余/失效）+ 删完重测，清理本票新增注释；改动与其它 fix 一并留工作树、仍不 commit。**判据在 skill 里，不在此复述。**
