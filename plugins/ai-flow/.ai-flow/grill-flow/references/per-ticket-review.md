...
2. **Standards 轴子代理**（`model='opus'`）：携 `references/fowler-smells.md` 全文 + 该 ticket 未提交 diff（`git diff`），**只报告 simplify 修不动的判断型 smell**（架构级重复、错 altitude、过度工程）。report-only 不 apply。这是 mattpocock 的质量轴，别退化成 correctness。
...
9. **写 qc marker**：在 tickets.md 该条加 `qc:done` 子标记。
...
- **串行**：绝不并行派实施子代理（避免两个子代理改同一文件的 race）；耦合的改动应在 stage-2 切进同一 ticket。
