...
| **持久产物自包含** | 代码注释 / 对外 commit message / 写入 context 层的知识（CLAUDE.md / rules / ADR）面向没有本次 session、也不翻 docs/feat-flows/ 的未来读者，**不得引用 flow 内部临时指代**（`Task N` / `U<k>` / `Phase X` / 「见上文」/ `design.md D-xx` / ADR 编号裸引用 等）——要表达就展开成实质内容。flow 内归档（task-reports.md / 开发者汇总表）不受限 |
...
| **禁止子代理隔离 worktree** | 任何 stage 派发子代理（`Agent` 工具）时，不使用 `isolation:"worktree"`。feat-flow 全程假定单一共享工作目录（docs commit 基准、Stage 4 起的 `base_sha_code` 框定代码 diff 全部依赖这个假设），孤立的 worktree 不会被后续 stage 感知，且若子代理有过改动可能不会被自动清理，变成游离分支/工作树留在仓库里 |
