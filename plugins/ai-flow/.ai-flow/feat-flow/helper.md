...
| **持久产物自包含** | 代码注释 / 对外 commit message / 写入 context 层的知识（CLAUDE.md / rules / ADR）面向没有本次 session、也不翻 plan.md 的未来读者，**不得引用 flow 内部临时指代**（`Task N` / `U<k>` / `Phase X` / 「见上文」等）——要表达就展开成实质内容。flow 内归档（task-reports.md / 开发者汇总表）不受限 |
