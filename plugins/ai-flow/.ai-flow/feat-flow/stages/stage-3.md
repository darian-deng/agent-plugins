...
本页只留步骤骨架，和那几条「违反了不会有任何东西变红」的红线。文件都在 `{{flow_def}}/references/` 下：
...
- 内部 review 三轮流程走完（Round 3 满足 `plan-review.md` 的跳过条件时可省），语义维度 1–3 + 结构门 4–7 **七项全部无残留问题**（或分歧已落盘、开发者已逐条决策、plan.md 已按决策更新）
