...
**状态**: 完成 | 完成但有顾虑
**Commit**: <commit-sha>
**验证**: verify `<命令>` → 通过 N 个 · 范围测试 `<实际跑的命令>` → 通过 N 个（⚠️ 任一 N 为 0 = 没验到东西，不是全绿；子代理没给实际命令就是没跑，补问再落盘）
**日期**: YYYY-MM-DD
...
- **测试完整性核查**：diff 里凡涉及测试文件（`*.test.*` / `*.spec.*` 等项目实际命名模式），逐条确认断言数量/强度相对改动前没有减少。减少但该 task 的状态不是「完成但有顾虑」（且 `### 遗留顾虑` 没写明理由）→ FAIL——这四条之前都只查「改没改声明外的东西」，这一条单独查「声明内的测试文件本身有没有被悄悄弱化」。
