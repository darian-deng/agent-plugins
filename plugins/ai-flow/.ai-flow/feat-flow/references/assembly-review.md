...
6. **测试调整复核（无条件跑，不等 review.md 先有记录）**：先跑 `git diff <base>..HEAD -- '<项目实际测试文件命名模式，如 **/*.test.* **/*.spec.*>'`，把命中的测试文件改动与 review.md「测试调整记录」逐条对表。**有测试文件 diff 但记录里没有对应条目 → 直接列为阻塞项**（未披露的测试改动，不必先判断改得对不对）。记录齐全的再验证：被改的测试是否真在测实现细节而非行为？新测试是否仍覆盖原意图？不成立同样列为阻塞项。
7. **实现深度（altitude）**：改动是在正确深度实现，还是把特例堆在共享基础设施上当创可贴？多个 task 都在加特例 = 底层机制该泛化的信号——作为**建议项**提（非阻塞，呈开发者裁决，可能 YAGNI，不进 3 轮循环）。
8. 局部 bug 只在上述视角下**顺手撞见**才报，不主动地毯式扫（测试文件改动例外——已被维度 6 无条件覆盖，不落在这条豁免里）。
