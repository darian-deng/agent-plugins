#!/bin/sh
# feat-flow2 preflight — runs once when 'feat-flow2 start' is called.
# Exit 0 = all checks pass. Non-zero = blocked with error message.
# cwd is .ai-flow/feat-flow2/
...
if ! check_cmd claude; then
  err "claude CLI not found. feat-flow2 only runs inside Claude Code."
...
if ! check_cmd git; then
  err "git not found. feat-flow2 requires git for BASE_SHA_CODE tracking and per-task commits."
  exit $FAIL
fi
...
if check_plugin "feature-dev"; then
  ...
  err "feat-flow2 will fail at Stage 1, 2, and 5 without this plugin."
  exit $FAIL
fi
...
if check_plugin "claude-md-management"; then
  ...
  err "feat-flow2 will fail at Stage 6 without this plugin."
  exit $FAIL
fi
