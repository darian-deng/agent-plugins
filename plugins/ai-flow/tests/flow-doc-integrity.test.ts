...
const SPLIT_STAGES: SplitStage[] = [
  {
    ...
    resident: [
      ...
      // 契约串在 2026-09 追加了 AC 交付清单：文件数 + `status --porcelain` 非空这两条机械判据
      // 对「9 条 AC 只交了 3 条」是瞎的（实测全绿放过）。正则只钉到「未提交」为止，后面允许追加——
      // 否则契约每长一点这条红线就误报，而误报几次之后它会被放宽成钉不住任何东西。
      ['实施代理的首行判据 impl-done:', /首行必须是\s*`impl-done: <N> 个文件已改，未提交/],
      ['实施代理首行判据带 AC 交付清单', /AC 已交 <编号列表\|全部> \/ 未交 <编号列表\|无>/],
