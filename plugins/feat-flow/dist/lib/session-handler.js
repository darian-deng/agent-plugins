...
export async function handleSessionStart(input) {
    ...
    if (!state) {
        const out = {
            ...
            additionalContext: '[feat-flow 警告] .feat-flow-active marker 存在但 state.json 不存在。\n' +
    ...
    // Build context
    const lines = [];
    lines.push(`[feat-flow] 正在继续 flow: ${state.flow_id}`, `当前阶段: ${state.current_stage}`);
    if (state.waiting_for_gate) {
        ...
        lines.push('', `⏳ 等待 GATE 审批（${state.gate_type}）`, `gate_context: ${state.gate_context ?? ''}`, '', `请用户执行：feat-flow approve <token>`, `如弹窗已关闭，可执行：! cat ${paths(cwd).gateToken}`);
    }
    else {
    ...
    lines.push('', `如需了解 feat-flow 工作流规则，参见：${HELPER_PATH}`);
    const out = {
        hookEventName: 'SessionStart',
