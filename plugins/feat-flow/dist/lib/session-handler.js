...
import { STAGES_DIR } from './config.js';
export async function handleSessionStart(input) {
    ...
    // Build context
    const lines = [];
    lines.push(`**feat-flow** 工作流恢复中`, ``, `- Flow: \`${state.flow_id}\``, `- 阶段: **${state.current_stage}**`);
    if (state.waiting_for_gate) {
        ...
        lines.push(``, `⏳ 等待 GATE 审批（${state.gate_type === 'task' ? '任务级' : '阶段级'}）`, ``, `请等待用户执行：\`feat-flow approve <token>\``, `Token 查看：\`! cat ${paths(cwd).gateToken}\``);
    }
    else {
