...
import type { PostToolInput, HookOutput, PostToolOutput, FeatFlowState, GateType, StageId } from './types.js';
import { NEXT_STAGE } from './types.js';
import {
  readState, writeState, hasActiveFlow, paths,
  writeGateToken, appendTransition, advanceStage,
} from './state.js';
...
function hasAnchors(content: string, ...anchors: string[]): boolean {
...
type GateDecision =
  | { triggered: false }
  | { triggered: true; type: GateType; context: string };
...
function checkCompletion(
  ...
  switch (stage) {
    ...
    case 'stage-6': {
      if (
        existsSync(join(verDir, 'lint.txt')) &&
...
const GATED_STAGES = new Set<StageId>(['stage-1', 'stage-3', 'stage-4', 'stage-7']);

function needsGate(stage: StageId, decision: Extract<GateDecision, { triggered: true }>): boolean {
  if (decision.type === 'task') return true;
...
export async function handlePostToolUse(input: PostToolInput): Promise<HookOutput | null> {
  ...
  if (state.waiting_for_gate) return null; // no re-trigger

  const writtenFile = (tool_input['file_path'] ?? '') as string;
  ...
  if (decision.triggered) {
    ...
    if (gated) {
      ...
      const updatedState: FeatFlowState = {
        ...state,
      ...
      let stageContent = '';
      try {
        if (existsSync(nextStageDoc)) stageContent = '\n\n' + readFileSync(nextStageDoc, 'utf-8');
      } catch { /* ignore */ }
