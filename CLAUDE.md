...
按提交前缀定，不要凭惯性接着上一版 +0.01：

- `fix` / `docs` / `chore` → **patch**（`0.79.0` → `0.79.1`）
- `feat` / `refactor!` → **minor**（`0.79.1` → `0.80.0`）

修复带上行为契约变化（比如某个判据从此更严、某条以前被拒的操作从此放行）仍然算 `fix`，走 patch；要 minor 得是新增能力或破坏性重构。

`claude plugin update` 只看版本号变没变，patch 和 minor 对它完全等价——所以没有任何机制会纠正走错的位，只能靠这条规则。真实记录：`v0.60.0` 到 `v0.79.0` 这 19 个版本全走了 minor，其中 9 个提交前缀是 `fix`，最后一次 patch bump 停在 `v0.59.1`。
