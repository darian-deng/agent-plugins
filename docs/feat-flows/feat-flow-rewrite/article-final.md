...
有段时间我靠一个越写越长的 skill 文件来约束 AI 的行为。一开始效果还行，代码风格对，命名遵守了，不该改的文件没动。直到某天跑一个中大型需求，context 窗口用到一半，skill 文件的那些指令好像从来没写过。AI 开始重新发明已经决策好的方案，改了不该改的文件，输出物越来越随机。我花了一个下午看哪里出了问题，最后得出的结论很难接受：文件本身没有问题，提示词写得也不差，是这件事根本不该用提示词来解决。
...
更要命的是，context 衰减和需求复杂度是正相关的。一个真正值得认真对待的需求，Stage 1 的技术对齐就得跑掉大量 context：讨论技术选型、确认边界、对齐 UI 状态、澄清隐性约束，用到 30% 是完全正常的。这些对齐对话有真实价值，但代价是把 skill 指令越推越深，而且这些对话会一直带着进后续每一轮，不断压缩可用空间。

还有一个问题不在技术层面，而在协作层面：团队里没有人能按同样的方式用 AI。每个人都有自己的一套"提示词直觉"，规范文档没人看完，就算看了也不知道 AI 这次到底在不在遵守。提示词层的东西是建议，不是约束。只要 AI 能绕过，它就会绕过。

需要一个不在提示词层工作的系统，一个在 AI 每次执行写操作时都能拦截并判断"这件事现在该不该做"的系统。

## 设计原则：取舍的尺子

两条原则在开始写代码之前就定了。

**质量第一，不追求降低 block 率。** 流程里的强制暂停和人工审批会影响体感，但这套设计允许它以质量为由存在。是刻意的取舍，后面所有的 gate 机制都是这条原则的产物。
...
每个阶段都有对应的落盘文件。用桌面端增加全局快捷键功能这个真实需求感受一下，这个功能涉及音频打点、截图打点、文本打点，Electron 应用，Jotai 状态管理：
...
PreToolUse hook 在 AI 发起任何写工具调用（Edit/Write/NotebookEdit）时拦截。如果当前 stage 是 `docs_only`（Stage 1-3），AI 想写代码文件会被直接 deny，不是"建议不要写"，是物理层面拒绝执行。Signal 文件的写入也在这里拦截：如果当前 stage 有 gate，AI 写 signal，引擎 deny 并生成一个随机 token 发给用户，只有用户执行 `feat-flow approve <token>` 才能让流程前进。AI 无法伪造 token，也无法绕过。

PostToolUse hook 在每次写操作完成后检查 context 使用率。阈值（feat-flow 专属）：≥ 30% 首次提醒，每再增加 5% 再提醒一次；≥ 60% 写工具被 block，所有后续写操作被 deny，必须 /clear。

为什么选 60%？在中大型需求里，Stage 1 的技术对齐对话用掉 30% 是正常的。再往后 context 增长很快，等到 80% 才 block 就太晚了，AI 的行为已经因为 context rot 开始漂移。宁可早一点 block，保证后续每一轮都从清洁的 context 开始。
...
这里用的是 grill-me，不是 brainstorming。brainstorming 是发散的，主动帮你想你没想到的方向；grill-me 是收敛的，一次问一个问题，强迫你给出明确答案。PM 提需求的场景通常需要收敛，需求已经有了，要做的是澄清、对齐、找出隐性约束。
...
code-architect 产出的是具体文件路径、接口签名、build 顺序这些东西。design.md 告诉你做什么，architecture.md 告诉你怎么做，后者直接决定了 plan.md 里的每个 task 能不能落地。Stage 2 完成后向用户输出 7 点审批清单（覆盖度、模块定位、接口设计、数据流、集成点、build 顺序、TDD bootstrap 完整性），Gate 之前主动呈现，不是等用户问。

**Stage 3：实施计划**

用 writing-plans skill 而不是让 AI 自由拆解，原因是结构化输出质量更稳定。writing-plans 默认就是 vertical slicing，一个 task 一个完整的 red-green 循环，而不是先写所有测试、再写所有实现这种 horizontal slicing。vertical slicing 保证每个 task 结束时代码处于可验证的稳定状态，不会出现"写到一半整个 build 都 broken"的情况。

**Stage 4：代码实施**

subagent-driven-development（SDD）负责逐 task 调度实施。这里做了一个和 SDD 默认行为不同的选择：不让主会话反复为每个 task 构造 architectural context，而是给每个 implementer subagent 一个文件清单（design.md / architecture.md / plan.md / 相关 ADR），让它自己按需读。主会话每次 dispatch 前，把已完成 task 的新术语作为 Pending vocabulary 注入下一个 implementer，确保后续 task 能看到前面 task 沉淀的命名约定，避免同一个概念在不同 task 里叫了不同名字。

**Stage 5：质量门**

把自动化验证（lint / typecheck / 全量测试）和代码审查合并成一个 stage。如果拆开，验证发现问题要修，修完重新验证，验证完去审查，审查发现问题再修，又要重新验证，两个 gate 审的是同一轮改动。合并后一个 stage 一个 gate，更干净。
...
Stage 6 很容易被跳过，也很容易被做成走形式。核心是让这次 flow 之后，项目的 AI coding 能力是净正向的，不只是新增，还有修复过时的 CLAUDE.md 规则、退役已被否定的决策记录、把这次实施中发现的新 pattern 写进 rules。
...
`feat-flow start` 时执行 preflight 检测，不是在中途才发现缺了什么。检测清单：claude CLI、Node.js ≥ 18、git、6 个必需 skill（grill-me / writing-plans / subagent-driven-development / receiving-code-review / optimize-claude-context / adr-manage）、feature-dev plugin。任何一项缺失就 fail，明确告诉你怎么安装。
...
Stage 1-3 的 `docs_only` 配置让 AI 在前三个阶段物理上无法写代码文件，不是靠提示词说"请不要修改代码"，是 PreToolUse hook 直接 deny 掉写代码文件的请求。
...
`/ai-flow:create`：从头创建一个自己的 flow。你描述想要的工作流是什么，AI 会引导你把这个流程翻译成 ai-flow 的格式——config.json（stage 定义和 gate 配置）、preflight.sh（环境检测）、每个 stage 的 prompt 文件。你不需要学会 ai-flow 的配置格式，只需要描述你想要什么流程，AI 帮你生成符合最佳实践的配置。这不是填表单，是真正的对话式配置生成。
...
我接受这套流程比"随便写"慢一些，前几个 stage 有 gate 会打断节奏。但在中大型需求上，那些 gate 后来证明是在对的时候问了对的问题。质量第一，block 率可以让步，这不是因为不在乎效率，而是那些 block 本来就该有，只是以前没有机制强制它发生。
